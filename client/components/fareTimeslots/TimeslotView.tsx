...
function TimeslotView({
  ...
  return (
    ...
    >
      {scheduleDayOfWeek.map((sdow, index) => {
        const startPercent = timeToPercent(sdow.startTime);
        const endPercent = timeToPercent(sdow.endTime);
        ...
        return (
          <Tooltip
            ...
            title={
              use12HourClock
                ? dayjs(sdow.startTime, "HH:mm:ss").format("h:mm:ss A") +
                  "-" +
                  dayjs(sdow.endTime, "HH:mm:ss").format("h:mm:ss A")
                : sdow.startTime + "-" + sdow.endTime
            }
            placement="top"
          >
            <Box
