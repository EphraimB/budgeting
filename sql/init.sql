...
BEGIN
    FOR pay_period IN SELECT make_date(extract(year from current_date)::integer, extract(month from current_date)::integer, s2.payroll_start_day::integer) AS start_date,
        ...
        LOOP
          ...
          EXECUTE format('SELECT cron.schedule(%L, %L, %L)',
            'payroll-' || pay_period.employee_id,
            cron_expression,
            format($$INSERT INTO transaction_history (account_id, transaction_amount, transaction_tax_rate, transaction_title, transaction_description, date_created, date_modified) 
                   VALUES ((SELECT account_id FROM employee WHERE employee_id = %s), %s, 0, 'Payroll', 'Payroll for %s to %s', current_timestamp, current_timestamp)$$,
                   pay_period.employee_id, pay_period.net_pay, pay_period.start_date, pay_period.end_date));
        END LOOP;
END;
$$ LANGUAGE plpgsql;
