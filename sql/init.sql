...
CREATE OR REPLACE FUNCTION process_payroll_for_employee(employee_id integer) RETURNS void AS $$
DECLARE
    pay_period RECORD;
...
BEGIN
    FOR pay_period IN 
        SELECT 
            ...
            SUM(COALESCE(e.regular_hours * e.hourly_rate * work_days, 0))::numeric(20, 2) AS gross_pay,
            SUM(COALESCE(e.regular_hours * e.hourly_rate * (1 - COALESCE(pt.rate, 0)) * work_days, 0))::numeric(20, 2) AS net_pay,
            SUM(COALESCE(e.regular_hours * work_days, 0))::numeric(20, 2) AS hours_worked
        FROM 
            employee e
            CROSS JOIN LATERAL (SELECT payroll_start_day, CASE 
                                ...
                                FROM payroll_dates) s2
            CROSS JOIN LATERAL (SELECT s2.payroll_start_day, CASE
                                    ...
                                    THEN s2.unadjusted_payroll_end_day - 2
                                    ...
                                    THEN s2.unadjusted_payroll_end_day - 1
                                    ELSE s2.unadjusted_payroll_end_day
                                END::integer AS adjusted_payroll_end_day
                                FROM payroll_dates) s1
            CROSS JOIN LATERAL (SELECT generate_series(start_date, end_date, '1 day') AS dates) AS dates
            CROSS JOIN LATERAL (SELECT SUM(CASE 
                                    WHEN (work_schedule::integer & (1 << (6 - extract(dow from dates.dates)::integer))) <> 0
                                    THEN 1 
                                    ELSE 0 
                                END) AS work_days
                                FROM employee e) s
            LEFT JOIN (SELECT employee_id, SUM(rate) AS rate
                       FROM payroll_taxes
                       GROUP BY employee_id) pt ON e.employee_id = pt.employee_id
        WHERE 
            e.employee_id = employee_id AND s.work_days <> 0
        GROUP BY 
            s2.payroll_start_day, e.employee_id, s.work_days, s1.adjusted_payroll_end_day
        ORDER BY 
            start_date, end_date
    LOOP
        cron_expression := '0 0 ' || EXTRACT(DAY FROM pay_period.end_date) || ' ' || EXTRACT(MONTH FROM pay_period.end_date) || ' *';

        inner_sql := format('INSERT INTO transaction_history (account_id, transaction_amount, transaction_tax_rate, transaction_title, transaction_description, date_created, date_modified) VALUES ((SELECT account_id FROM employee WHERE employee_id = %L), %L, 0, ''Payroll'', ''Payroll for %s to %s'', current_timestamp, current_timestamp)',
                            employee_id, pay_period.net_pay, pay_period.start_date, pay_period.end_date);

        EXECUTE format('SELECT cron.schedule(%L, %L, %L)',
            'payroll-' || employee_id || '-' || pay_period.start_date || '-' || pay_period.end_date,
            cron_expression,
            inner_sql);
    END LOOP;
END;
