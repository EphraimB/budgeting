const generate2YearlyExpenses = (transactions, expense, toDate) => {
    ...
    while (expenseDate <= toDate) {
        ...
        if (expense.frequency_day_of_week) {
            let firstDate = new Date(expenseDate.getFullYear() + 2, expenseDate.getMonth(), expense.frequency_week_of_month !== null ? 1 + (7 * (expense.frequency_week_of_month)) : expense.expense_begin_date.getDate());
            ...
            expenseDate.setFullYear(expenseDate.getFullYear() + 2);
        }
    }
};

module.exports = generate2YearlyExpenses;