const accountQueries = {
  ...
  const expenseQueries = {
    getExpensesByAccount: "SELECT * FROM expenses WHERE account_id = $1 AND expense_begin_date <= expense_begin_date + INTERVAL '1 month'::interval * $2 ORDER BY expense_begin_date ASC",
  ...
  const currentBalanceQueries = {
    getCurrentBalance: "SELECT accounts.account_id, COALESCE(accounts.account_balance::money, '0') + COALESCE(SUM(deposits.deposit_amount::money), '0') - COALESCE(SUM(withdrawals.withdrawal_amount::money), '0') AS account_balance FROM accounts LEFT JOIN deposits ON accounts.account_id = deposits.account_id LEFT JOIN withdrawals ON accounts.account_id = withdrawals.account_id WHERE accounts.account_id = $1 GROUP BY accounts.account_id",
  };
