...
export const getWishlists = async (
    ...
    try {
        ...
        if (accountId) {
            query = `
                ...
                FROM 
                    get_generated_transactions($2) WITH (wishlist) w
                WHERE account_id = $1
                ORDER BY 
                    w.priority ASC;
            `;
            params = [accountId, dayjs().add(5, 'year')];
        } else {
            query = `
    ...
    FROM 
        recurring r
    WHERE
        (r.date + interval '1 day') <= '2025-12-31'
        AND (r.remaining_balance IS NULL OR r.remaining_balance - ABS(r.amount) > 0)
            ...
            CROSS JOIN LATERAL generate_series(current_date, '2025-12-31'::date + INTERVAL '1 month', '1 month') AS d1(date)
            CROSS JOIN LATERAL (
                SELECT
                    CASE WHEN
